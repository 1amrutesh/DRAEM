...
class MVTecDRAEMTrainDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        anomaly_source_idx = torch.randint(
            0, len(self.anomaly_source_paths), (1,)
        ).item()
        (
            image,
            augmented_image,
...
class MIADDRAEMTestDataset(Dataset):
    def __init__(self, root_dir, classes, resize_shape=None):
        self.root_dir = root_dir
        self.resize_shape = resize_shape
        self.classes = classes
        self.images = []
        for cls in self.classes:
            class_test_dir = os.path.join(root_dir, cls, "test")
            for sub_dir in ["good"] + [
                f.name
                for f in os.scandir(class_test_dir)
                if f.is_dir() and f.name != "good"
            ]:
                self.images.extend(
                    sorted(
                        glob.glob(
                            os.path.join(class_test_dir, sub_dir, "*.png")
                        )
                    )
                )
...
class MIADDRAEMTrainDataset(Dataset):
    def __init__(
        self, root_dir, classes, anomaly_source_path, resize_shape=None
    ):
        self.root_dir = root_dir
        self.resize_shape = resize_shape
        self.classes = classes
        self.image_paths = []
        for cls in self.classes:
            class_train_dir = os.path.join(root_dir, cls, "train", "good")
            self.image_paths.extend(
                sorted(glob.glob(os.path.join(class_train_dir, "*.png")))
            )
        self.anomaly_source_paths = sorted(
    ...
    def transform_image(self, image_path, anomaly_source_path):
        ...
        anomaly_idx = np.random.randint(0, len(self.anomaly_source_paths))
        anomaly_source_path = self.anomaly_source_paths[anomaly_idx]
        augmented_image, anomaly_mask, has_anomaly = self.augment_image(
            image, anomaly_source_path
        )
        augmented_image = np.transpose(augmented_image, (2, 0, 1))
    ...
    def __getitem__(self, idx):
        ...
        (
            ...
            self.image_paths[idx], self.anomaly_source_paths
        )
        sample = {
            "image": image,
            "anomaly_mask": anomaly_mask,
