import torch
from data_loader import MVTecDRAEMTrainDataset, MIADDRAEMTrainDataset
...
def train_on_device(obj_names, args):
    ...
    for obj_name in obj_names:
        ...
        dataset = MIADDRAEMTrainDataset(
            root_dir=args.data_path,
            anomaly_source_path=os.path.join(
                args.anomaly_source_path, obj_name
            ),
            resize_shape=[256, 256],
        )

        dataloader = DataLoader(
            dataset, batch_size=args.bs, shuffle=True, num_workers=16
        )
            ...
            for i_batch, sample_batched in enumerate(dataloader):
                ...
                if args.visualize and n_iter % 200 == 0:
                    ...
                    visualizer.plot_loss(
                        ssim_loss, n_iter, loss_name="ssim_loss"
                    )
                    visualizer.plot_loss(
                        segment_loss, n_iter, loss_name="segment_loss"
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--gpu_id", action="store", type=int, default=0, required=False
    )
    parser.add_argument("--data_path", action="store", type=str, required=True)
    parser.add_argument(
        "--anomaly_source_path", action="store", type=str, required=True
    )
    parser.add_argument(
