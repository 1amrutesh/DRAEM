...
from data_loader import MVTecDRAEMTestDataset, MIADDRAEMTestDataset
...
def test(obj_names, miad_path, checkpoint_path, base_model_name):
    obj_ap_pixel_list = []
    ...
    for obj_name in obj_names:
        ...
        dataset = MIADDRAEMTestDataset(
            miad_path + obj_name + "/test/", resize_shape=[img_dim, img_dim]
        )
        ...
        for i_batch, sample_batched in enumerate(dataloader):
            ...
            true_mask_cv = (
                true_mask.detach().numpy()[0, :, :, :].transpose((1, 2, 0))
            )
        ...
        total_gt_pixel_scores = total_gt_pixel_scores[
            : img_dim * img_dim * mask_cnt
        ]
        total_pixel_scores = total_pixel_scores[: img_dim * img_dim * mask_cnt]
        auroc_pixel = roc_auc_score(total_gt_pixel_scores, total_pixel_scores)
        ap_pixel = average_precision_score(
            total_gt_pixel_scores, total_pixel_scores
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--base_model_name", action="store", type=str, required=True
    )
    parser.add_argument("--data_path", action="store", type=str, required=True)
    parser.add_argument(
        "--checkpoint_path", action="store", type=str, required=True
    )
    ...
    if args.dataset == "mvtec":
        obj_list = mvtec_obj_list
    elif args.dataset == "miad":
        obj_list = miad_obj_list
